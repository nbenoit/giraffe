...
public abstract class OutputUI extends Node implements IONode
{
    ...
    public OutputUI ( String img_normal_name, String img_selected_name )
    {
        super ( );
    ...
    {
        Output out = new Output ( this.getBitsCount(), c.getCircuit() );

        for ( iterAnchors=this.anchors.iterator(); iterAnchors.hasNext(); )
            {
                Anchor a = iterAnchors.next ( );
                a.linkForSimulation ( out.getWire(a.getID()), c );
            }
